...
def generate_contract_detail_html(case_data: dict, include_screenshot_js: bool = False) -> str:
    ...
    if include_screenshot_js:
        ...
        <script>
        // Wait for EVERYTHING to fully load - AGGRESSIVE WAITING STRATEGY
        let pageFullyLoaded = false;
        let button = null;
        let statusDiv = null;
        
        window.addEventListener('load', function() {
            button = document.getElementById('screenshotBtn');
            ...
            // Wait aggressively for all resources + fonts + rendering
            setTimeout(() => {
                // Check if fonts are really loaded
                if (document.fonts) {
                    document.fonts.ready.then(() => {
                        // Extra wait after fonts ready
                        setTimeout(() => {
                            ...
                            console.log('✅ Page fully ready for screenshot');
                        }, 1000);
                    });
                } else {
        ...
        function captureAndCopyToClipboard() {
            ...
            if (!pageFullyLoaded) {
                if (statusDiv) {
                    statusDiv.textContent = '⏳ Please wait, page still loading...';
...
def ensure_payments_columns():
    """Ensure payments table has approval/proof columns (runtime safety)."""
    conn = sqlite3.connect(DB_PATH, timeout=30)
    conn.row_factory = sqlite3.Row
    try:
        conn.execute("PRAGMA journal_mode=WAL;")
    ...
    def _add(name, ddl):
        if name not in cols:
            conn.execute(ddl)
    try:
        _add('proof_image_drive_id', "ALTER TABLE payments ADD COLUMN proof_image_drive_id TEXT")
        _add('proof_image_filename', "ALTER TABLE payments ADD COLUMN proof_image_filename TEXT")
        _add('approval_status', "ALTER TABLE payments ADD COLUMN approval_status TEXT DEFAULT 'pending'")
        _add('approval_by', "ALTER TABLE payments ADD COLUMN approval_by TEXT")
        _add('approval_at', "ALTER TABLE payments ADD COLUMN approval_at TEXT")
        _add('rejection_notes', "ALTER TABLE payments ADD COLUMN rejection_notes TEXT")
        conn.commit()
    except Exception:
        pass
    conn.close()
...
MENU_ITEMS = [
    ...
    {"label": "Utility",    "page": "Utility", "roles": ("Superuser", "Supervisor"), "primary": False},
...
def main():
    ...
    if st.session_state.page == "Supervisor":
        page_supervisor()
        return
    if st.session_state.page == "Utility":
        page_utility()
        return
            ...
            if not sel:
                ...
                with st.form("agent_update_supervisor_fields"):
                    ...
                    with csa:
                        ...
                        # Paid Off field - hanya bisa diubah oleh Supervisor/Superuser
...
def page_supervisor():
    ...
    with tabs[2]:
        ...
        # Assignment controls - NEW SIMPLIFIED SYSTEM
        st.markdown("---")
        st.subheader("📋 Assign Data ke Tracer")
        ...
        btn_assign = st.button(
            ...
            type="primary",
            disabled=(not selected_tracers or total_available == 0),
            use_container_width=True,
            key="btn_assign_distribute"
        )
        ...
        if btn_assign:
            if df_sup is None or df_sup.empty:
                st.error("Tidak ada data untuk di-assign.")
            elif not selected_tracers:
                ...
                try:
                    ...
                    # Get data to assign (limit to num_to_assign)
                    data_to_assign = df_sup.head(num_to_assign).to_dict(orient="records")
                    
                    # Distribute data
                    counts = {t: 0 for t in selected_tracers}
                    frozen = 0
                    already_assigned = 0
                    success_count = 0
                    ...
                    tracer_idx = 0
                    for idx, r in enumerate(data_to_assign):
                        progress_percent = int(((idx + 1) / len(data_to_assign)) * 100)
                        progress_bar.progress(progress_percent / 100)
                        status_text.markdown(f"🔄 **Memproses {idx + 1} dari {len(data_to_assign)}** ({progress_percent}%)")
                        ...
                        # Assign to tracer (round-robin style)
                        assignee = selected_tracers[tracer_idx % len(selected_tracers)]
                        tracer_idx += 1
                    ...
                    progress_bar.progress(100)
                    status_text.markdown("✅ **Proses selesai!**")
                    
                    # Audit log
                    try:
                        distribution_detail = ", ".join([f"{t}: {c}" for t, c in counts.items()])
                        execute(
                            "INSERT INTO audit_logs (user_id, action, details) VALUES (?,?,?)",
                            (
                                u.get('id'),
                                "TRACE_ASSIGN_DISTRIBUTE",
                                f"Assigned {success_count} cases to {len(selected_tracers)} tracers. Distribution: {distribution_detail}. Frozen: {frozen}, Already assigned: {already_assigned}"
                            )
                        )
                    except Exception:
                        pass
                    ...
                    # Show summary
                    st.success(f"✅ Berhasil assign: **{success_count}** case")
                    
                    # Show distribution
                    dist_cols = st.columns(len(selected_tracers))
                    for idx, (tracer, count) in enumerate(counts.items()):
                        with dist_cols[idx]:
                            st.metric(tracer, count)
                    
                    if frozen > 0:
                        st.warning(f"❄️ Frozen: {frozen}")
                    if already_assigned > 0:
                        st.info(f"🔒 Already assigned: {already_assigned}")
    ...
    with tabs[3]:
        ...
        with st.expander("🔍 Filter Lanjutan", expanded=False):
            ...
            # Pre-load cached options (hindari loading saat interaksi dropdown)
            ...
            if 'aa_active_lending_filter' not in st.session_state:
                st.session_state['aa_active_lending_filter'] = "-- Semua Lending Entity --"
            if 'aa_active_employment_filter' not in st.session_state:
                st.session_state['aa_active_employment_filter'] = []
            
            # Form untuk filter (hindari loading sebelum klik Set Filter)
            with st.form("form_advanced_filter", clear_on_submit=False):
                fcol1, fcol2, fcol_btn = st.columns([2, 2, 1])
                
                with fcol1:
                    filter_lending_entity = st.selectbox(
                        "Filter by Lending Entity / Product",
                        options=le_options,
                        index=le_options.index(st.session_state['aa_active_lending_filter']) if st.session_state['aa_active_lending_filter'] in le_options else 0,
                        key="aa_lending_entity_input",
                ...
                with fcol2:
                    filter_employment = st.multiselect(
                        ...
                        options=emp_options,
                        default=st.session_state['aa_active_employment_filter'],
                        key="aa_employment_input",
                        help="Filter berdasarkan status employment (DEBTOR/SPOUSE/dll) - berguna untuk produk tertentu seperti AkuLaku."
                    )
                
                with fcol_btn:
                    st.markdown("<div style='margin-top: 25px;'></div>", unsafe_allow_html=True)
                    submit_filter = st.form_submit_button("🔍 Set Filter", type="primary")
                
                # Handle form submission
                if submit_filter:
                    st.session_state['aa_active_lending_filter'] = filter_lending_entity
                    st.session_state['aa_active_employment_filter'] = filter_employment
                    # Clear cache untuk trigger refresh data
                    if 'aa_df_cache' in st.session_state:
                        del st.session_state['aa_df_cache']
                    if 'aa_cache_key' in st.session_state:
                        del st.session_state['aa_cache_key']
                    st.rerun()
            
            # Show active filter info
            selected_lending_entity = st.session_state['aa_active_lending_filter']
            selected_employment = st.session_state['aa_active_employment_filter']
...
def page_utility():
    require_roles(("Superuser", "Supervisor"))
    st.title("Utility Menu")
    
    # Initialize database connection for this page
    conn = get_db()
    
    tabs = st.tabs(["Payment Recap", "Trace Results", "Enriched & Lookup", "Freeze Manager", "Company Library", "Data Migration"])
